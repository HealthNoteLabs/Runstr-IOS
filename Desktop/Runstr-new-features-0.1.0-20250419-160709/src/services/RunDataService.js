...
class RunDataService {
  ...
  saveRun(runData) {
    try {
      ...
      const id = runData.id || Date.now() + '-' + Math.random().toString(36).substr(2, 9);
      const date = runData.date || new Date().toLocaleDateString();
      
      // Ensure we have both splits formats
      const distanceUnit = localStorage.getItem('distanceUnit') || 'km';
      
      // If we only have one splits format, generate the other
      let splitsKm = runData.splitsKm || [];
      let splitsMi = runData.splitsMi || [];
      
      // If we have the splits but not the unit-specific ones, create them
      if (runData.splits && (!runData.splitsKm || !runData.splitsMi)) {
        if (distanceUnit === 'km') {
          splitsKm = [...runData.splits];
          splitsMi = convertSplitsToMiles(runData.splits, 'km');
        } else {
          splitsMi = [...runData.splits];
          splitsKm = convertSplitsToKm(runData.splits, 'mi');
        }
      }
      
      // Create the complete run object
      const newRun = {
        id,
        date,
        ...
        ...runData,
        splits: runData.splits || [], // Keep for compatibility
        splitsKm,
        splitsMi
      };
