...
class RunTracker extends EventEmitter {
  ...
  async start(options = {}) {
    ...
    // Update activityType from options or localStorage
    this.activityType = options.activityType || localStorage.getItem('activityMode') || ACTIVITY_TYPES.RUN;
    ...
    this.pace = 0;
    this.splits = [];
    this.splitsKm = []; // Initialize km splits
    this.splitsMi = []; // Initialize mile splits
  ...
  restoreTracking(savedState) {
    ...
    this.splitsKm = [...savedState.splitsKm];
    this.splitsMi = [...savedState.splitsMi];
    this.elevation = {
  ...
  handleSplit(newSplit, distanceUnit) {
    // Add to the main splits array
    this.splits.push(newSplit);
    
    // Update both km and mile formats
    if (distanceUnit === 'km') {
      this.splitsKm = [...this.splits];
      this.splitsMi = convertSplitsToMiles(this.splits, 'km');
    } else {
      this.splitsMi = [...this.splits];
      this.splitsKm = convertSplitsToKm(this.splits, 'mi');
    }
    
    // Emit event with all splits data
    this.emit('splitRecorded', {
      splits: this.splits,
      splitsKm: this.splitsKm,
      splitsMi: this.splitsMi
