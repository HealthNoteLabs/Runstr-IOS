...
export const createRunContentForNostr = (run, distanceUnit) => {
  if (!run) {
    throw new Error('No run data provided');
  }
  
  // Use the appropriate splits based on the current unit preference
  const splitsToPublish = distanceUnit === 'km'
    ? (run.splitsKm || run.splits) // Fallback to regular splits if splitsKm not available
    : (run.splitsMi || run.splits); // Fallback to regular splits if splitsMi not available
    
  // Create a copy of the run with the appropriate splits
  const runWithCorrectSplits = {
    ...run,
    splits: splitsToPublish
  };
  
  return JSON.stringify(runWithCorrectSplits);
};

export const createAndPublishRunEvent = async (run, distanceUnit = 'km') => {
  const eventContent = createRunContentForNostr(run, distanceUnit);
