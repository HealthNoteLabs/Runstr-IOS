...
/**
 * Convert splits from miles to kilometers
 * @param {Array} splits - Array of split objects
 * @param {string} fromUnit - Original unit ('km' or 'mi')
 * @returns {Array} - Converted splits
 */
export function convertSplitsToKm(splits, fromUnit = 'mi') {
  if (!splits || !splits.length) return [];
  if (fromUnit === 'km') return [...splits];
  
  return splits.map(split => ({
    ...split,
    km: split.km, // Keep original split number for consistency
    pace: split.pace * 0.621371, // Convert mi/km pace ratio
  }));
}

/**
 * Convert splits from kilometers to miles
 * @param {Array} splits - Array of split objects
 * @param {string} fromUnit - Original unit ('km' or 'mi')
 * @returns {Array} - Converted splits
 */
export function convertSplitsToMiles(splits, fromUnit = 'km') {
  if (!splits || !splits.length) return [];
  if (fromUnit === 'mi') return [...splits];
  
  return splits.map(split => ({
    ...split,
    km: split.km, // Keep original split number for consistency
    pace: split.pace * 1.609344, // Convert km/mi pace ratio
  }));
}
