...
import { createAndPublishEvent, createWorkoutEvent, createAndPublishRunEvent } from '../utils/nostr';
...
export const RunHistory = () => {
  ...
  const [postedRuns, setPostedRuns] = useState(new Set());
  ...
  const handlePostToNostr = async (run) => {
    ...
    try {
      // Use the new function that handles splits correctly
      await createAndPublishRunEvent(run, distanceUnit);
      
      // Update posted runs tracking
      const updatedPostedRuns = new Set(postedRuns);
      updatedPostedRuns.add(run.id);
      setPostedRuns(updatedPostedRuns);
      
      alert('Run posted to Nostr successfully!');
    } catch (error) {
      console.error('Error posting to Nostr:', error);
      alert('Failed to post run to Nostr. Please try again.');
    } finally {
      setIsPosting(false);
            ...
            return (
              <li key={run.id} className="history-item">
                <RunHistoryCard
                  ...
                  isWorkoutSaved={workoutSavedRuns.has(run.id)}
                  ...
                  isPosting={isPosting && currentRunId === run.id}
                  isPosted={postedRuns.has(run.id)}
                />
              </li>
            );
          })}
